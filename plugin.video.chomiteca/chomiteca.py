...
listURL = [MainURL, ChomikMainURL]
nameURL = ['Minhateca', 'Chomikuj']
usernameURL = [urllib.quote(selfAddon.getSetting('minhateca-username')), urllib.quote(selfAddon.getSetting('chomikuj-username'))]
...
def login(defora=False):
	...
	for x in range(0, len(username)):
		...
		if re.search('003eA senha indicada n',logintest):
			...
			entrarnovamente(1)
			selfAddon.setSetting(label[x]+'-check',"false")
		elif re.search('Certifique se que indicou o nome correto.',logintest):
			mensagemok(label[x],traducao(40003))
			entrarnovamente(1)
			selfAddon.setSetting(label[x]+'-check',"false")
		elif re.search('"LoggedIn":true',logintest):
...
def menu_principal(ligacao):
	if ligacao==1:
		...
		for x in range(0, len(listURL)):
			if ReturnStatus(nameURL[x].lower()): addDir(traducao(40009)+nameURL[x],listURL[x] + usernameURL[x],3,wtpath + art + 'my.png',2,True)
		for x in range(0, len(listURL)):
			if ReturnStatus(nameURL[x].lower()): addDir(traducao(40010)+nameURL[x],'pastas',5,wtpath + art + 'goto.png',2,True)
...
def proxpesquisa_ck():
    ...
    conteudo= net.http_POST(endlogin,form_data=form_d,headers=ref_data).content.encode('latin-1','ignore')
    addon.save_data('temp.txt',form_d)
    pastas(ChomikMainURL + 'action/nada','coco',conteudo=conteudo)
...
def pastas(url,name,formcont={},conteudo='',past=False,deFora=False,listagem=False):
	...
	if re.search('ProtectedFolderChomikLogin',conteudo):
		...
		teste= net.http_POST(endlogin,form_data=form_d,headers=ref_data).content.encode('latin-1','ignore')
		teste=urllib.unquote(teste)
		if re.search('<span class="field-validation-error">A password introduzida',teste): mensagemok('Minhateca',traducao(40002))
		elif re.search('<div class="ui-tooltip-titlebar">',teste): mensagemok('Chomikuj',traducao(40002))
		else: pastas_ref(url)
	else:
		try:
			  if host == 'minhateca.com.br': conta=re.compile('<div class="bigFileInfoRight">.+?<h3>(.+?)<span>(.+?)</span></h3>').findall(conteudo)[0]
			  else: conta=re.compile('<div class="fileInfoFrame borderRadius">.+?<span class="bold">.+?</span>.+?<span class="bold">(.+?)</span>\s+(.+?)</p>').findall(conteudo)[0]
...
def ReturnConteudo(conteudo,past,color,url2,deFora):
	...
	for part in section:
		...
		if not name: name = re.compile('<span class="bold">(.+?)</span>....\s+</a>', re.DOTALL).findall(part)
...
def caixadetexto(url,ftype=''):
      ultpes=''
      save=False
      if url=='pastas' and re.search('Minhateca',name): title=traducao(40010) + " Minhateca"
      elif url=='pastas' and re.search('Chomikuj',name): title=traducao(40010) + "Chomikuj"
      ...
      if (keyb.isConfirmed()):
            ...
            if url=='pastas' and re.search('Minhateca',name): pastas(MainURL + search,name)
            elif url=='pastas' and re.search('Chomikuj',name): pastas(ChomikMainURL + search,name) 
				...
				if ReturnStatus('chomikuj'):
					...
					pastas(ChomikMainURL + 'action/SearchFiles',name,formcont=form_d,past=True)
      else: sys.exit(0)
...
if mode==None or url==None or len(url)<1:
	if selfAddon.getSetting('minhateca-enable') == 'false' and selfAddon.getSetting('chomikuj-enable') == 'false':
		...
		if (selfAddon.getSetting('minhateca-enable') == 'true' and not selfAddon.getSetting('minhateca-username')== '') or (selfAddon.getSetting('chomikuj-enable') == 'true' and not selfAddon.getSetting('chomikuj-username')== '') : login()
		menu_principal(1)
